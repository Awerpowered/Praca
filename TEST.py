...
def dopisz_dane_do_arkusza(gc, nazwa_arkusza_matki, nazwa_zakladki_wynikowej, dataframe_wynikow):
    """
    Zapisuje dane do arkusza wynikowego. Znajduje pierwszy wolny wiersz i wkleja tam dane.
    Jeśli arkusz jest pusty, dodaje również nagłówki.
    """
    ...
    try:
        print(f"--- Rozpoczęto proces zapisu do arkusza '{nazwa_zakladki_wynikowej}' ---")
        ...
        # Znajdź następny wolny wiersz. Jest to najprostsza i najbardziej niezawodna metoda.
        all_values = zakladka.get_all_values()
        next_row = len(all_values) + 1

        # Przygotuj dane do zapisu
        data_to_write = []
        # Jeśli arkusz jest pusty (next_row będzie 1), dodaj najpierw nagłówki
        if next_row == 1:
        ...
        # Zaktualizuj arkusz za pomocą metody update, zaczynając od pierwszego wolnego wiersza
        print(f"Zapisywanie {len(data_to_write)} wierszy, zaczynając od komórki A{next_row}...")
        zakladka.update(f'A{next_row}', data_to_write, value_input_option='USER_ENTERED')

        print(f"✅ Operacja zapisu została pomyślnie zakończona dla '{nazwa_zakladki_wynikowej}'.")
        return True

    except gspread.exceptions.APIError as e:
        print(f"❌ KRYTYCZNY BŁĄD API Google podczas operacji zapisu: {e}")
        # Wypisz szczegółową odpowiedź z API, jeśli jest dostępna
        print(f"   Szczegóły błędu: {e.response.text}")
        return False
    except Exception as e:
        print(f"❌ KRYTYCZNY BŁĄD podczas operacji zapisu: {e}")
        return False
...
def main():
    ...
    # --- DODATKOWE LOGOWANIE DANYCH ---
    print("\n--- Przygotowano dane do zapisu ---")
    print(f"Liczba rekordów do zapisania: {len(wyniki_df)}")
    if not wyniki_df.empty:
        print("Dane:")
        # Używamy to_string(), aby wyświetlić całą ramkę danych w logach
        print(wyniki_df.to_string())
    else:
